...
namespace
{
  ...
  const uint64_t TEST_MONEY_SUPPLY = (uint64_t)(-1);
  ...
  const size_t testGrantedFullRewardZoneV1 = CRYPTONOTE_BLOCK_GRANTED_FULL_REWARD_ZONE_V1;
  const size_t testGrantedFullRewardZoneV2 = CRYPTONOTE_BLOCK_GRANTED_FULL_REWARD_ZONE_V2;
  ...
  protected:
    ...
    void do_testV1(uint64_t alreadyGeneratedCoins, uint64_t fee) {
      m_blockTooBig = !m_currency.getBlockReward(BLOCK_MAJOR_VERSION_1, testMedianV1, testBlockSizeV1, alreadyGeneratedCoins, fee, m_blockReward, m_emissionChange);
    }

    void do_testV2(uint64_t alreadyGeneratedCoins, uint64_t fee) {
      m_blockTooBig = !m_currency.getBlockReward(BLOCK_MAJOR_VERSION_2, testMedianV2, testBlockSizeV2, alreadyGeneratedCoins, fee, m_blockReward, m_emissionChange);
    }
