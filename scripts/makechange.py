...
Open the wallets with marcoin-serivce like so:

./marcoin-service -w walletA.wallet -p yourpass --rpc-password test --bind-port 8070
./marcoin-service -w walletB.wallet -p yourpass --rpc-password test --bind-port 8071
...
moveDecimal = 100  # H2O has 2 decimals so 100 is the divide/multiply factor
minAmount = 100 * moveDecimal  # min number for amount to xfer
maxAmount = 5000 * moveDecimal  # max number for amount to xfer
anonymity = 3
fee = 10 # atomic units, H2O would be 0.10 as the tx network fee
